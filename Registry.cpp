...
CRegStdString& CRegStdString::operator =(stdstring s){
 m_value = s;
 if (!m_force)
   return *this;   //no write to the registry required
 write();
 return *this;
}
...
CRegStdWORD& CRegStdWORD::operator=(DWORD dw){
 m_value = dw;
 if (!m_force)
   return *this;   //no write to the registry required, its the same value
 write();
 return *this;
}
