...
void myLog::init(const string& fileName, int mode)
{
   if ( (fileName.c_str())[0] )
      ...
      openLog("syslog.txt", mode);
}

void myLog::init()
{
    openLog("c:\\MyProjects\\Holoprog\\Debug\\syslog.txt",LOG_WRITE);
    //openLog("syslog.txt",LOG_WRITE);
}
...
void myLog::openLog(const string& fileName, int mode)
{
   if (logLevel < QUIET_MODE)
   {
      ...
      {
         MessageBox(NULL,"Above named Logfile could not be opened!",fileName.c_str(),MB_OK);
...
//    *this << line_2 << endl; I've tried removing the endl, but it is needed (at least sometimes)
      *this << line_2 << endl;
*/
      CTime time = CTime::GetCurrentTime();
	  CString sStatus;
      sStatus = time.Format(_T("Date: %B %d, %Y"));
      *this << sStatus;
	  sStatus.Format("      ttTime: %s", time.Format("%H:%M:%S             ") );
      *this << sStatus;
      *this << logName << endl;
      if (theEnd) *this << getExecTime();
   }        
} 
