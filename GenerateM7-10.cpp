...
// it, uses it, then forgets it.  GenerateProfileForM12()does the same... Its just working storage.  To conserve memory, I'll use this same space for M7,8,9&10 too... Here it's not an Array Of GonioAngles, but of
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//   M07     M07     M07     M07     M07     M07     M07     M07     M07     M07     M07     M07     M07     M07     M07  //
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//   M07     M07     M07     M07     M07     M07     M07     M07     M07     M07     M07     M07     M07     M07     M07  //
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
int GenerateProfileForM7(HWND hDlg,
 ...
 //For this next structure, the Finish Position is given (i.e. Provided by the Radcliffe spreadsheets) and is the motor position required for pixel zero on a given line.
...
#define SIZ_ARR_SFS_M12  2000
 ...
 int nLeadInForM39 = (nLengthOfM39rowsIncLeadInOut_inPixels + 1 - nHCFrecNoofCols) / 2;    //This is to check that the calculation succeeded - i.e. was possible in the time allowed.
 ...
 char szConstructedFileName[12] = "AccProfM07-";
 if(idxMnoP1 == 19){
    szConstructedFileName[8] = '1'; //change szConstructedFileName to "AccProfM19-";
    szConstructedFileName[9] = '9';
   }
 else if(idxMnoP1 == 31){
    szConstructedFileName[8] = '3'; //change szConstructedFileName to "AccProfM31-";
    szConstructedFileName[9] = '1';
   }
 StringCchCat(szAccelFileNamesDir,_MAX_DIR,szConstructedFileName);//originally ,"AccProfM07-");
...
/*if(nHCFrecNoofLines == 123456789){  //This is solely to help when debugging, so should be removed. The parameter 123456789 tells to return after loading the file
 ...
 if(LoadAndCheckAccelProfileFile(hDlg, szAccelFileNamesDir, idxMnoP1, nAccProfile, chHCFrecNoofUsteps4EachMotorFromTab, nHCFrecFrequencyOfPixelPrintsInHz))
   return 1000002;// <-- L oadAndCheckAccelProfileFile failed
/*
...
*/
 if(myOpen_File_VariableAp_CSV("C:\\Documents and Settings\\Just Me\\My Documents\\FromExcelM7r.csv", nHCFrecNoofLines, nHCFrecNoofCols))
   return 1000003;

 int nNoofUstepsPerDegree = PCFrec.chNoofUsteps4EachMotorTab[idxMno] * GONIO_STEPS_PER_DEGREE; //GONIO_STEPS_PER_DEGREE is 100 because 1 revolution is 2 degrees
 ...
 //Then Calculate the Penultimate and Ultimate Pixel
   ...
   //From M39:
    ...
    if(nLengthOfCalculatedMotorMovementInPixPeriods > nLeadInForM39){
     /*int nLength_mes;
       memset( mes, 0, SIZMES);
       ...
       _itoa_s(nLeadInForM39, mes + nLength_mes, SIZMES - nLength_mes, 10);
    ...
    ArrSFS[y].nTotalPixelNoM11A = nFoundMinNoofElements;//-this needn't be stored on the 'first pass' (& is likely to be increased anyway)
...
//***
    ...
    ArrSFS[y].nEndTotalPixelNoD = nFoundMinNoofElements;//-this needn't be stored on the 'first pass' (& is likely to be increased anyway)
...
// settable through the user interface, which provides flexibility.  It will also be easier, in future, to automatically generate its value (if nNOOFm11prePIXzeroCONSTspeedPIXELS is explicit in the code).
    //nNoofLeadingZeroMovs = nLeadInForM39 - ArrSFS[y].nTotalPixelNoM11A - nNOOFm11prePIXzeroCONSTspeedPIXELS;
    ...
    nTotalPixelNoM11 = nLeadInForM39 - ArrSFS[y].nTotalPixelNoM11A - nNOOFm11prePIXzeroCONSTspeedPIXELS + (y * MAXNOOFPIXELS_IN_ROW);
...
//int ACCEL_DECEL_PROFILE_SIZE = nFoundMinNoofElements + nNOOFprePIXzeroCONSTspeedPIXELS;
 int nPulsesThisPixel;
 int FirstLineLen = nLengthOfM39rowsIncLeadInOut_inPixels;  //for M39 this was int FirstLineLen = nHCFrecNoofCols - 1 + ACCEL_DECEL_PROFILE_SIZE * 2;
...
/*This almost certainly doesn't do anything anymore
 for(int nGetLeadIn_inUsteps = 0; nGetLeadIn_inUsteps < nLeadInForM39; nGetLeadIn_inUsteps++){
...
//For the Red Gonio:                                 NOTE THIS CODE IS DEPENDENT ON ALL THE LINES HAVING THE SAME START AND END POSITIONS AS EACH OTHER.
    if(nGONIO_COL_IDX == 0){
       for(nSPos = 0; nSPos < nHCFrecNoofLines; nSPos+= 2)
         nLineXstartPositionsFEM[idxMno][nSPos] = ArrSFS[y].nSpos;    //The data stored in nLineXstartPositionsFEM for motors M11 - M12 is relative to zero degrees.  If the user subsequently changes the offset from home of zero
...
int MustTestAboveCodeIsOK;
    ...
    if(nMaxPhysPos > PCFrec.nPhysLimMaxUsteps[idxMno]){
     ...
     //double dblStepsPerMM = (double)500.0f * HCFrec.HCF_PCFrecCopy.chNoofUsteps4EachMotorTab[eAchM] / XAXIS_PITCH; //500 is the steps per rev for the 5 phase stage motors.
       ...
       while(IDYES == MessageBox(hDlg,"Holoprog is unable to create valid motor positions based on this hologram configuration!\n\nMotor 11 (Gonio Vert) would have to move outside it's 'MAXIMUM Physical Limit'\nby the above number of micro steps (& degrees)\n\nDo you need suggestions?",mes,MB_YESNO)){
       */
       double dblDEGperStep  = 1.0f / nNoofUstepsPerDegree;
...
/*********************************************************** END OF CHECKING ***********************************************************/
 ...
 FillInHeaderForGeneratedFile(nHCFrecNoofLines + 2 * nNoofRowsBetweenBeams, nHCFrecNoofCols, nHCFrecFrequencyOfPixelPrintsInHz, nLeadInForM39, ptr_HCFrec_uTimeStamp, szHCFrecHologramName);
 ...
 mySave_File (hDlg, nRequiredSpace, "csv",TYPE5CSV, szLocalM11ProfileFileName); //returns zero if file saved OK  (Uses the global nLengthOfM39rowsIncLeadInOut_inPixels - set when generating M39)
 nchCurrentMotorZB = (char)idxMno;
